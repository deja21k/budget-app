...
export class InsightsService {
  ...
  private getTransactionsInRange(startDate: string, endDate: string): Transaction[] {
  ...
  private getIncomeInRange(startDate: string, endDate: string): Transaction[] {
    const stmt = this.db.prepare(`
      SELECT 
        t.*,
        c.name as category_name,
  ...
  private getExpensesInRange(startDate: string, endDate: string): Transaction[] {
    const stmt = this.db.prepare(`
      SELECT 
        t.*,
        c.name as category_name,
    ...
    for (const expense of expenses) {
      ...
      if (existing) {
        ...
        categoryMap.set(catId, {
          ...
          budget_limit: (expense as any).budget_limit,
   ...
   */
  private analyzeTimePatterns(expenses: Transaction[]): TimePattern[] {
    const dayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
    const patterns = new Map<number, { total: number; count: number }>();
   ...
   */
  private detectRepeatedExpenses(expenses: Transaction[]): RepeatedExpense[] {
  ...
  private calculateFrequency(transactions: Transaction[]): RepeatedExpense['frequency'] {
  ...
  private estimateMonthlyCost(frequency: RepeatedExpense['frequency'], avgAmount: number): number {
    switch (frequency) {
   ...
   */
  private analyzeRegret(expenses: Transaction[]): RegretAnalysis {
