...
const COMMON_ITEMS: Record<string, { minPrice: number; maxPrice: number }> = {
...
function findPriceRange(itemName: string): { minPrice: number; maxPrice: number } | null {
...
function estimatePrice(itemName: string, store: string): number | null {
...
export async function predictPrices(itemName: string): Promise<PricePrediction> {
