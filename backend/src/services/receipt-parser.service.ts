...
export class ReceiptParser {
  ...
  private isSerbianFiscalReceipt(lines: string[], text: string): boolean {
  ...
  private extractStoreName(lines: string[], isSerbianReceipt: boolean): string | null {
    if (isSerbianReceipt) {
  ...
  private extractSerbianStoreName(lines: string[]): string | null {
    for (let i = 0; i < Math.min(10, lines.length); i++) {
      const line = lines[i].trim();
      
      if (/^(фискални|фиск|рачун|fiskalni|fisk|racun|vreme)/i.test(line)) continue;
  ...
  private extractGenericStoreName(lines: string[]): string | null {
  ...
  private extractDate(text: string): string | null {
    ...
    const datePatterns = [
      { regex: /(\d{1,2})[\/\-\.](\d{1,2})[\/\-\.](\d{2,4})/, format: 'MDY' as const },
      { regex: /(\d{4})[\/\-\.](\d{1,2})[\/\-\.](\d{1,2})/, format: 'YMD' as const },
    ];
  ...
  private extractPIB(text: string): string | null {
    const pibPatterns = [
  ...
  private detectCurrency(text: string): string | null {
    ...
    for (const pattern of rsdPatterns) {
      if (pattern.test(textLower)) return 'RSD';
    }
    
    if (/\beur\b/i.test(textLower) || /\b€/.test(text)) return 'EUR';
    if (/\bgbp\b/i.test(textLower) || /£\d/.test(text)) return 'GBP';
    
    return 'USD';
  }

  private extractFiscalCode(text: string): string | null {
    const patterns = [
  ...
  private extractAmounts(lines: string[], isSerbianReceipt: boolean): { total: number | null; subtotal: number | null; tax: number | null } {
      ...
      if (!total) {
        ...
        if (matches.length > 0) {
          const amounts = matches
            ...
            .filter((a): a is number => !isNaN(a) && a > 0);
  ...
  private parseSerbianAmount(amountStr: string): number | null {
  ...
  private extractLineItems(lines: string[], isSerbianReceipt: boolean): LineItem[] {
    ...
    if (isSerbianReceipt) {
      for (let i = 0; i < lines.length; i++) {
        ...
        if (/^(фискални|рачун|уukupно|порез|пдв|датум|време)/i.test(line)) continue;
        if (line.length < 5 || /^\d{1,3}(\.\d{3})*,\d{2}$/.test(line)) continue;
